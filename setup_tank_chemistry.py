...
HEADERS = [
    ...
    "學理說明",        # 給使用者看的解釋
    "狀態",            # V / ? / 空白 (V=已核可, ?=待討論)
    "HRT_min (hr)",    # 水力停留時間下限 (空白 = 不檢查)
    "HRT_max (hr)",    # 水力停留時間上限
    "SOR_max (m³/m²·d)",  # 表面溢流率上限 (空白 = 不適用, 如非沉澱類)
    "G_min (1/s)",     # G 值下限
    "G_max (1/s)",     # G 值上限
]
...
# ──────────────────────────────────────────────────
RULES = [
    # 每筆: (標準槽體, 加藥類型, 應變動項目, 不應變動項目, 容忍度%, 嚴重度, 學理說明, 狀態,
    #        HRT_min, HRT_max, SOR_max, G_min, G_max)
    # HRT 單位: hr, SOR 單位: m³/m²·d, G 單位: 1/s
    # 空字串 "" = 不檢查

    # === pH 調整 / 中和類 ===
    ("pH調整槽",          "酸/鹼",               "pH, 水溫",              "*",
     5, "不合理", "純 pH 調整槽只加酸/鹼, 無分離機制, 其他水質應守恆", "V",
     0.25, 0.75, "", "", ""),  # 中和反應 15~45 分
    ("pH調整池",          "酸/鹼",               "pH, 水溫",              "*",
    ...
    ("pH調整暨快混池",    "酸鹼 + 混凝劑",       "pH, 水溫, SS",          "*",
     ...
     0.017, 0.05, "", 300, 1000),  # 含快混功能: 1~3 分, G 高
    ...
    ("快混槽",            "混凝劑",              "SS, 水溫",              "*",
     5, "不合理", "加 PAC/明礬/FeCl₃ 混凝, SS 增加合理, 但無沉降, "
                  "重金屬/COD/BOD 不該減少", "V",
     0.008, 0.033, "", 300, 1000),  # 0.5~2 分, G 300~1000
    ("快混池",            "混凝劑",              "SS, 水溫",              "*",
     5, "不合理", "同快混槽 (命名變體)", "V",
     0.008, 0.033, "", 300, 1000),
    ("慢混池",            "助凝劑",              "SS, 水溫",              "*",
     5, "不合理", "加助凝劑 (PAM) 形成大絮凝體, SS 可微增, 仍無沉降, "
                  "重金屬/COD/BOD 不該減少", "V",
     0.083, 0.5, "", 10, 50),  # 5~30 分, G 10~50 (T01-09 391 會被抓)
    ("慢混槽",            "助凝劑",              "SS, 水溫",              "*",
     5, "不合理", "同慢混池", "V",
     0.083, 0.5, "", 10, 50),
    ...
    ("離子交換樹脂塔",    "離子交換樹脂",        "指定離子 (Ca, Mg, Na, NO₃, 重金屬)", "pH, COD, BOD, SS",
     20, "待確認", "依樹脂類型 (陽/陰/螯合) 只去除特定離子, 對有機物/SS 無作用; "
    ...
    # === 脫水 / 濃縮 (特殊: 質量守恆優先) ===
    ("脫水機",            "脫水助劑",            "含水率",                "*",
     10, "待確認", "物理脫水, 濃度暴升合理但污染物質量應守恆; 系統會用質量基準檢查", "V",
     "", "", "", "", ""),
    ("濃縮槽",            "無 (重力濃縮)",       "含水率",                "*",
     10, "待確認", "重力濃縮污泥, 同脫水機原則", "V",
     "", "", 12, "", ""),  # 污泥濃縮 SOR ≤ 12
    ("污泥儲槽",          "無 (儲存)",           "含水率",                "*",
     15, "待確認", "暫存污泥, 質量應守恆", "V",
     "", "", "", "", ""),
    ...
    ("暫存槽",            "無 (均化)",           "*",                     "重金屬, 離子",
     30, "待確認", "均化多股廢水, 進出水質可變動但污染物質量應大致守恆; "
                   "若大幅減少需確認是否誤填", "V",
     6, 48, "", "", ""),  # 暫存 6~48 hr
    ("貯留槽",            "無 (均化)",           "*",                     "重金屬, 離子",
     30, "待確認", "同暫存槽", "V",
    ...
    ("調勻池",            "無 (均化)",           "*",                     "重金屬, 離子",
     ...
     6, 48, "", "", ""),
    ("廢水調整池",        "無 (均化)",           "*",                     "重金屬, 離子",
     30, "待確認", "均化多股廢水穩定後端進流, 重金屬等不應自行減少", "V",
     6, 48, "", "", ""),
    ("廢水收集池",        "無 (均化)",           "*",                     "重金屬, 離子",
     30, "待確認", "同廢水調整池", "V",
     "", 12, "", "", ""),  # 收集池可較短
    ...
    ("批次反應槽",        "視製程而定",          "視批次而定",            "視批次而定",
     20, "待確認", "依批次操作條件不同, 規則難以一概而論, 須人工判讀", "V",
     "", "", "", "", ""),
    ("放流池",            "無 (儲存)",           "(不應變動)",            "*",
...
def main():
    ...
    # 欄寬 (對應 13 欄: 原 8 欄 + HRT_min/max + SOR_max + G_min/max)
    widths = [22, 18, 32, 30, 10, 12, 50, 8, 10, 10, 14, 10, 10]
